...
class AIA_Admin_Dashboard {
    ...
    public function __construct() {
        ...
        add_action('wp_ajax_aia_delete_generated_post', array($this, 'ajax_delete_generated_post'));
    }
    ...
    public function render_dashboard() {
        ...
        <div class="wrap aia-dashboard">
            ...
            <div class="aia-quick-actions">
                ...
                <button type="button" id="aia-manual-generate" class="button button-secondary">Generate Next Post Now</button>
                <span id="aia-generate-status" class="aia-generate-status" style="margin-left:10px;"></span>
                ...
                <?php else: ?>
                    <span style="margin-left: 10px;">No pending keywords</span>
                <?php endif; ?>
            </div>
        ...
        jQuery(document).ready(function($) {
            ...
            $('#aia-manual-generate').on('click', function() {
                ...
                $.ajax({
                    ...
                    error: function(xhr) {
                        ...
                        status.removeClass().addClass('error').text('❌ ' + message);
                        button.prop('disabled', false);
                    }
                });
            });
            ...
            $('.aia-delete-post-btn').on('click', function() {
                var button = $(this);
                var postId = button.data('post-id');
                if (!postId) return;

                if (!confirm('Delete this generated post permanently? This will move the WordPress post to Trash.')) {
                    return;
                }
                ...
                $.ajax({
                    ...
                    dataType: 'json',
                    data: {
                        action: 'aia_delete_generated_post',
                        post_id: postId,
                        nonce: '<?php echo wp_create_nonce('aia_delete_generated_post'); ?>'
                    },
                    success: function(response) {
                        if (response.success) {
                            button.closest('tr').fadeOut(300, function() { $(this).remove(); });
                        } else {
                            alert(response.data && response.data.message ? response.data.message : 'Failed to delete post.');
                    ...
                    <?php foreach ($generated_posts as $post): ?>
                        <tr>
                            ...
                            <td>
                                ...
                                <button type="button" class="button button-small aia-delete-post-btn" data-post-id="<?php echo esc_attr($post->ID); ?>">
                                    Delete
                                </button>
                                <button type="button" 
    ...
    public function ajax_manual_generate() {
        ...
        $keywords_manager = new AIA_Keywords_Manager();
        $next = $keywords_manager->get_next_pending_keyword();

        if (!$next) {
            wp_send_json_error(array('message' => 'No pending keywords are available.'));
        }

        // Manual dashboard generation is direct/synchronous. It intentionally
        // bypasses the automatic daily-post limit and does not start a worker.
        $cron = new AIA_Cron_Handler_Async();
        $result = $cron->process_single_keyword_direct($next['index'], $next['data']);

        if (!empty($result['success'])) {
            wp_send_json_success(array(
                'message' => 'Post generated successfully for: ' . $next['data']['keyword'],
                'post_id' => $result['post_id']
            ));
        }

        wp_send_json_error(array(
            'message' => !empty($result['message']) ? $result['message'] : 'Failed to generate the post.'
        ));
    }
    ...
    public function ajax_regenerate_post() {
        ...
        try {
            ...
            // IMPORTANT: regeneration does not fetch or replace images.
            // The existing featured image and existing image URLs in the post are preserved.
    ...
    // ========== AJAX: DELETE GENERATED POST ==========
    public function ajax_delete_generated_post() {
        if (!isset($_POST['nonce']) || !wp_verify_nonce($_POST['nonce'], 'aia_delete_generated_post')) {
            wp_send_json_error(array('message' => 'Security check failed.'));
            return;
        }
        ...
        if (!get_post_meta($post_id, '_aia_generated', true)) {
            wp_send_json_error(array('message' => 'This is not a Blog Autom generated post.'));
            return;
        }

        $deleted = wp_trash_post($post_id);
        if (!$deleted) {
            wp_send_json_error(array('message' => 'Could not move the post to Trash.'));
            return;
        }

        $logger = new AIA_Logger();
        $logger->log("Generated post moved to Trash. Post ID: {$post_id}", 'info');
        wp_send_json_success(array('message' => 'Post moved to Trash.'));
    }
}